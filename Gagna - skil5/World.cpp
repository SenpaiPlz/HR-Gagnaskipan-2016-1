...
void World::createOrganisms(OrganismType orgType, int count) { // creates count organisms of type orgType
    ...
    while(i < count)
    {
        int tmpX = 0;
        int tmpY = 0;
        randomPosition(tmpX,tmpY);

        if(grid[tmpX][tmpY] == NULL)
        {
            i++;
            if(orgType == ANT)
            {
                new Ant(this,tmpX,tmpY);
            }
            else if(orgType == BUG)
            {
                new Bug(this,tmpX,tmpY);
            }
        }
    }
}
...
// to move it again when we reach it.
    ...
    for (int i = 0; i < WORLDSIZE; i++)
    {
        for (int j = 0; j < WORLDSIZE; j++)
        {
            ...
            //  If there's a creature set moved to false
            if(grid[i][j] != NULL)
            {
...
void World::moveOrganism(OrganismType aType) { // Move all organisms of type aType
    ...
    for (int i = 0; i < WORLDSIZE; i++)
    {
        ...
        {
            //  Iterate through the world
            //  If the creatures match the type and has not moved
...
void World::breedOrganisms() { // Make the organisms breed
    ...
    for (int i = 0; i < WORLDSIZE; i++)
    {
        ...
        {
            //  Iterate through the world
            //  If there's a creature call the breed function.
            if(grid[i][j] != NULL)
            {
                grid[i][j]->breed();
            }
